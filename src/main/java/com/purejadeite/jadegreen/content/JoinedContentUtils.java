...
public class JoinedContentUtils {
	 ...
	 * @return 対象の値リスト
	 */
	public static List<Content<?>> getKeyContents(WorkbookContent book, JoinedCellDefinition<?> definition) {
		Definition<?> sheetKeyDefinition = definition.getKeyDefinition();
	 ...
	 * @return 対象の値
	 */
	public static Content<?> getMyKeyContent(WorksheetContent sheet, JoinedCellDefinition<?> definition) {
		Definition<?> sheetKeyDefinition = definition.getMyKeyDefinition();
	 ...
	 * @return シート
	 */
	public static WorksheetContent getTargetSheet(Content<?> myKeyContent, List<Content<?>> keyContents) {
		...
		for (Content<?> keyContent : keyContents) {
			if (myKeyContent != keyContent && myKeyValues.equals(keyContent.getValues())) {
				if (sheetContent != null) {
					// キーが一意でないならば例外
					throw new ContentException("結合対象のシートが複数存在します", keyContent);
				}
				sheetContent = keyContent.getUpperContent(WorksheetContent.class);
			}
		}
		return sheetContent;
	}
